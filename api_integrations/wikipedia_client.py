...
class WikipediaClient:
        ...
        try:
            ...
            for article in articles:
                formatted_article = {
                    ...
                    'lang': article.get('lang', language)
                }
    ...
    def check_common_knowledge(self, text: str) -> Dict[str, Any]:
        """
        Check if text contains common knowledge that should be filtered from plagiarism detection
        ...
        try:
                ...
                if key_terms:
                    ...
                    for term in key_terms[:2]:  # Limit to top 2 terms per sentence
                        ...
                        for article in articles:
                            ...
                            similarity = self._calculate_similarity(sentence, article['extract'])
            ...
            common_knowledge_ratio = len(common_knowledge_sentences) / max(total_sentences, 1)
            ...
            return {
                ...
                'should_filter': common_knowledge_ratio > 0.1,  # Filter if >10% common knowledge
                'filter_suggestions': self._generate_filter_suggestions(common_knowledge_sentences)
            }
    ...
    def get_scientific_concepts(self, field: str, limit: int = 20) -> List[Dict]:
        """
        ...
        try:
            ...
            for article in articles:
                ...
                if any(keyword in article['title'].lower() for keyword in ['concept', 'theory', 'principle', 'law', 'method']):
                    concepts.append({
        ...
        for statement in statements:
            try:
                ...
                for term in key_terms[:3]:  # Check top 3 terms
                    ...
                    for article in articles:
                        similarity = self._calculate_similarity(statement, article['extract'])
                ...
                if validation_result['wikipedia_matches']:
                    ...
                    validation_result['confidence'] = max(match['similarity'] for match in validation_result['wikipedia_matches'])
    ...
    def _extract_key_terms(self, sentence: str) -> List[str]:
        ...
        for word in words:
            ...
            if any(tech in word.lower() for tech in ['algorithm', 'method', 'theory', 'system', 'process']):
                key_terms.append(word)
    ...
    def _is_potential_fact(self, sentence: str) -> bool:
        ...
        fact_indicators = [
            ...
            'discovered', 'found', 'developed', 'created'
        ]
    ...
    def _generate_filter_suggestions(self, common_knowledge_sentences: List[Dict]) -> List[str]:
        ...
        if common_knowledge_sentences:
            suggestions.append(f"Found {len(common_knowledge_sentences)} sentences that match common knowledge")
            suggestions.append("Consider excluding these from plagiarism detection")
            ...
            if unique_topics:
                suggestions.append(f"Common knowledge topics: {', '.join(unique_topics[:3])}")
...
def test_wikipedia():
    ...
    print(f"Common knowledge sentences found: {len(common_knowledge['common_knowledge_sentences'])}")
