...
class SemanticSimilarity:
    ...
    def calculate_similarity(
        ...
        try:
            ...
            doc_sentences = [
                s.strip() for s in doc_sentences if s and s.strip()
            ]
            ref_sentences = [
                s.strip() for s in ref_sentences if s and s.strip()
            ]
            ...
            # Encode sentences
            logger.info(
                f"Encoding {len(doc_sentences)} document and {len(ref_sentences)} reference sentences"
            )
            ...
            result = SemanticResult(
                ...
                per_sentence_scores=[
                    round(score, 4) for score in per_sentence_scores
                ],
            ...
            logger.error(
                f"Semantic similarity calculation error: {e}", exc_info=True
            )
            raise PlagiarismDetectionError(
    ...
    def calculate_similarity_legacy(
        self, doc_sentences: List[str], ref_sentences: List[str]
    ) -> float:
        """
